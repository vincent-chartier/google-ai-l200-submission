...
class OutingCoordinatorService:
    ...
    async def get_or_create_session_async(self, session_id: str) -> Dict[str, Any]:
        """Asynchronously retrieves or initializes session state from memory cache or persistent SQLite database."""
        if session_id in self.sessions:
            return self.sessions[session_id]
    ...
    async def _run_async_memory_pipeline(
        self,
        session_id: str,
        user_turn: Optional[Dict[str, Any]],
        agent_turn: Optional[Dict[str, Any]],
        session_state: Dict[str, Any]
    ) -> None:
        """Background asynchronous memory worker.
        
        Executes without blocking client responses:
        1. Async persistence of dialogue turns to SQLite.
        2. Async persistence of session state (seen, favorites, active booking) to SQLite.
        3. Real-time background entity extraction (discovering new favorites/seen movies).
        4. Automatic evaluation of history compaction threshold and async summarization.
        5. Evaluation metrics logging.
        """
        try:
            # 1. Async turn persistence to SQLite
            if user_turn:
                await self.db.append_turn(session_id, user_turn)
            if agent_turn:
                await self.db.append_turn(session_id, agent_turn)

            # 2. Async session state persistence to SQLite
            await self.db.save_session(session_id, session_state)

            # 3. Async profile entity extraction
            turns_to_scan = [t for t in [user_turn, agent_turn] if t]
            if turns_to_scan:
                extracted = self.housekeeping_service.summarizer.extract_entities_from_turns(
                    turns=turns_to_scan,
                    session_state=session_state
                )
                if extracted.get("favorites") or extracted.get("seen"):
                    await self.db.save_session(session_id, session_state)

            # 4. Automatic background history compaction when threshold is reached
            history = session_state.get("conversation_history", [])
            tokens = estimate_history_tokens(history, session_state.get("conversation_summary", ""))
            if len(history) > 4 or tokens > 350:
                compact_res = await self.housekeeping_service.compact_conversation_history_async(
                    session_state=session_state,
                    max_recent_turns=4,
                    token_threshold=350,
                ...
                if compact_res.get("compacted"):
                    self.compaction_evaluator.evaluate_compaction(
                        tokens_before=compact_res["tokens_before"],
                        tokens_after=compact_res["tokens_after"],
                        turns_compacted=compact_res["turns_compacted_count"],
                        remaining_turns=compact_res["remaining_turns_count"]
                    )
                    logger.info(
                        f"Background compaction completed for {session_id}: "
                        f"compacted {compact_res['turns_compacted_count']} turns, saved {compact_res['tokens_saved']} tokens."
                    )
        except Exception as e:
            logger.warning(f"Background async memory pipeline error for {session_id}: {e}")

    def schedule_background_memory_pipeline(
        self,
        session_id: str,
        user_turn: Optional[Dict[str, Any]],
        agent_turn: Optional[Dict[str, Any]],
        ...
        """Dispatches an asynchronous background task for turn persistence, entity extraction, and history compaction."""
        if background_tasks is not None and hasattr(background_tasks, "add_task"):
            background_tasks.add_task(
                self._run_async_memory_pipeline,
                session_id,
                user_turn,
                agent_turn,
                session_state
            ...
            try:
                ...
                asyncio.run(self._run_async_memory_pipeline(
                    session_id=session_id,
                    user_turn=user_turn,
                    agent_turn=agent_turn,
                    session_state=session_state
    ...
    def _enqueue_memory_task(
        ...
        self.schedule_background_memory_pipeline(
            session_id=session_id,
            user_turn=turn_data if role == "user" else None,
            agent_turn=turn_data if role != "user" else None,
            session_state=session_state
        )
    ...
    async def handle_user_message(
        ...
        # 2. Add incoming user turn to memory
        user_turn = add_conversation_turn(session_state, role="user", text=sanitized_message, sender="user")
        ...
        # 5. Record agent response in memory and schedule background async persistence & compaction
        agent_turn = add_conversation_turn(
            session_state,
            role="agent",
            text=response.text,
    ...
    async def handle_a2ui_action(
        ...
        user_turn = add_conversation_turn(session_state, role="user", text=action_repr, sender="user")
        ...
        # Record agent action response in history & schedule background async persistence & compaction
        agent_turn = add_conversation_turn(
            session_state,
            role="agent",
            text=response.text,
